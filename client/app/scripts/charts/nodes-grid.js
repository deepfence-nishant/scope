...
import {
  getNodeSeverityColor,
  getNodeSeverityStrokeColor
...
function renderIdCell({ rank, label, labelMinor, pseudo, metadata, nodeSeverity }) {
  ...
  let colorNode = getNodeSeverityColor(label, nodeSeverity);
  let nodeStroke = getNodeSeverityStrokeColor(label, nodeSeverity);

  if (typeof metadata != 'undefined') {
    for (var i=0; i < metadata.length; i++) {
      if (metadata[i].id == 'high_severity') {
        if (parseInt(metadata[i].value) > 0) {
          colorNode = colorMapper['red'];
          break;
        }
      } else if (metadata[i].id == 'medium_severity') {
        if (parseInt(metadata[i].value) > 0) {
          colorNode = colorMapper['yellow'];
          break;
        }
      }
    }
...
class NodesGrid extends React.Component {
  ...
  render() {
    const { nodes, height, gridSortedBy, gridSortedDesc, canvasMargins,
      searchNodeMatches, searchQuery, nodeSeverity } = this.props;
    const cmpStyle = {
      height,
