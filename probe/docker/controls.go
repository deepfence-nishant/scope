...
const (
    ...
    /*
     * Shyam - Uncomment this to provide controls for container
	LsContainer      = "docker_ls_container"
    */
...
/*
 * Shyam - Uncomment this to provide controls for container
func (r *registry) lsContainer(containerID string, req xfer.Request) xfer.Response {
	exec, err := r.client.CreateExec(docker_client.CreateExecOptions{
		AttachStdin:  true,
		AttachStdout: true,
		AttachStderr: true,
		Tty:          true,
		Cmd:          []string{"/bin/sh", "-c", "/bin/ls -l /;echo \"Press any key to exit\";read"},
		Container:    containerID,
	})
	if err != nil {
		return xfer.ResponseError(err)
...
func (r *registry) registerControls() {
	controls := map[string]xfer.ControlHandlerFunc{
        ...
        /*
         * Shyam - Uncomment this to provide controls for container
		LsContainer:      captureContainerID(r.lsContainer),
        */
